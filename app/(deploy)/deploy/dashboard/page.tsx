    ...
    <div className="flex flex-col h-full bg-gradient-to-br from-background via-background to-muted/20 font-sans">
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6 relative">
        ...
        <div className="max-w-7xl mx-auto space-y-6 relative">
          {/* Welcome Banner */}
          <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-card via-muted/20 to-card shadow-sm hover:shadow-md transition-shadow duration-200">
          ...
          <div className="grid gap-4 md:gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {quickActions.map((action, index) => (
              <Card 
                key={index} 
                className="cursor-pointer hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] border border-border/60 bg-card backdrop-blur-sm group"
          ...
          <div className="grid gap-4 md:gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-6">
              {/* Payment Setup Card */}
              <Card className="border border-border/60 bg-gradient-to-br from-card via-accent/10 to-card shadow-sm">
                <CardContent className="p-4 md:p-6">
                  <div className="flex items-start justify-between">
                    <div className="space-y-3">
                      <div>
                        <h3 className="font-semibold text-foreground mb-1 tracking-tight">Set Payment Method</h3>
          ...
          <Card className="border border-border/60 shadow-sm bg-card/80 backdrop-blur-sm">
            ...
            <CardContent>
              {recentJobs.length > 0 ? (
                ...
                <div className="text-center py-8">
                  <div className="mb-2">
                    <span className="text-xs bg-muted px-2 py-1 rounded font-mono">ft-m9xzxyui-ot05x</span>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {modelLibrary.map((model) => (
                <Card 
                  key={model.id} 
                  className="cursor-pointer hover:shadow-xl hover:shadow-primary/20 transition-all duration-300 hover:-translate-y-2 hover:scale-[1.02] border border-border/60 bg-card backdrop-blur-sm group overflow-hidden hover:border-primary/30"
                  onClick={() => router.push(`/deploy/new?model=${model.id}`)}
                >
                  <CardContent className="p-0">
                    {/* Clean Model Header - matches Fireworks style */}
                    <div className="p-4 bg-card border-b border-border/20">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {model.featured && (
                          <Badge className="bg-primary text-primary-foreground border-0 font-medium text-xs">
